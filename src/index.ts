...
export default {
  async fetch(req: Request, env: Bindings, ctx: ExecutionContext): Promise<Response> {
    ...
    if (missing.length) {
      console.error(`[FATAL] rald-search: missing required secrets: ${missing.join(', ')}`);
      return new Response(JSON.stringify({ error: 'Service misconfigured', missing, service: 'rald-search' }), {
